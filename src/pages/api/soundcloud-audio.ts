// Next.js API route for SoundCloud audio extraction (works with pnpm dev)
...
export default async function handler(
  ...
  try {
    ...
    if (!url) {
      return res.status(400).json({ error: 'SoundCloud URL required' });
    }

    // Validate SoundCloud URL
    const soundcloudPattern = /^https?:\/\/(www\.)?soundcloud\.com\/[\w-]+\/[\w-]+/;
    if (!soundcloudPattern.test(url)) {
      return res.status(400).json({ error: 'Invalid SoundCloud URL format' });
    }

    // Find yt-dlp in common locations (yt-dlp supports SoundCloud)
    const ytDlpPaths = [
    ...
    try {
      const metadataCommand = `"${ytDlpCmd}" --print "%(title)s|||%(uploader)s|||%(artist)s" --no-download "${url}"`;
    ...
    let audioPath = path.join(tmpDir, `soundcloud_${timestamp}_${randomId}.mp3`);

    try {
      // Try to extract audio as MP3 first (requires ffmpeg)
      let command = `"${ytDlpCmd}" -x --audio-format mp3 --audio-quality 0 -o "${audioPath}" "${url}"`;
      let stdout = '';
      let stderr = '';
      ...
      try {
        ...
        if (mp3Error.stderr && mp3Error.stderr.includes('ffmpeg') && mp3Error.stderr.includes('not found')) {
          ...
          command = `"${ytDlpCmd}" -f "bestaudio[ext=m4a]/bestaudio" -o "${audioPathNoExt}.%(ext)s" "${url}"`;
          ...
          try {
            ...
            const files = fs.readdirSync(tmpDir)
              .filter(f => f.startsWith(`soundcloud_${timestamp}_`) && f.includes(timestamp.toString()))
              .map(f => ({
                name: f,
      ...
      if (!fs.existsSync(audioPath)) {
        return res.status(500).json({ 
          ...
          message: stderr || 'Audio file was not created. The track may be private or unavailable.'
        });
      }
      ...
      res.setHeader('Content-Disposition', `attachment; filename="soundcloud_audio.mp3"`);
      ...
      return res.status(500).json({ 
        error: 'Failed to extract audio',
        message: execError?.stderr || execError?.message || 'Unknown error. The track may be private, unavailable, or require authentication.'
      });
    }
  } catch (error: any) {
    return res.status(500).json({ 
