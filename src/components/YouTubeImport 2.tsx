...
export const YouTubeImport: React.FC<YouTubeImportProps> = ({ onImport, onError }) => {
  ...
  const [statusMessage, setStatusMessage] = useState('Extracting audio from YouTube...');
  ...
  const handleImport = async () => {
    ...
    setIsLoading(true);
    setProgress(0);
    setStatusMessage('Requesting audio from YouTube...');
    ...
    try {
      ...
      try {
        setStatusMessage('Downloading audio from YouTube...');
